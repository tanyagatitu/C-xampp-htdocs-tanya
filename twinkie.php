  ...
  <thead>
    <?php for($i=0;$i<8;$i++):?>
      ...
      <!-- preg_replace(PATTERN,REPLACEMENT,$string) -->
        ...
        echo (preg_replace('/[()0-9>+=\/]+/', '',ucwords(strtolower($wce_array[$i])))) ; ?></th>
    <?php endfor; ?>
  </thead>
  <tbody>

    <?php for($j=1;$j<(count($wce_array)-8)/9;$j++):?>
    ...
    if ($i==3||$i==2||$i==5) {
      # code...
      $st=str_replace('oo', 'o',preg_replace("/[^a-zA-Z]/",'',ucwords(strtolower($wce_array[8*$j+$i]))));
    }else if ($i==7) {
      # code...
